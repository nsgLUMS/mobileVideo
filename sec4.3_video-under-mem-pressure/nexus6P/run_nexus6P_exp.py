...
def reach_memory_state(mem_state):
    ...
    while not ('stateMsg' in app_data and isStateReached(mem_state, app_data['stateMsg'])):        
        try:
            ...
            r = requests.post(f'http://{IP_ADDRESS}:{PORT}/halt-approach')
            print(f'Response: {r.status_code}, {r.text}')
            return False
        except Empty:
            time.sleep(2)
...
def kill_mem_app_process():
    proc = subprocess.Popen('adb shell top -n 1 | grep "com.example"', shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    ...
    for line in lines:
        ...
        if (match := re.match(r'^\d+', line)):
            ...
            run(f'adb shell su -c "kill -9 {pid}"')
        else:
            print(f'Cant extract pid from "{line}"')

def kill_mem_app():
    run('adb shell su -c "service call statusbar 1"')
    time.sleep(2)
    run("adb shell input tap 143 862")
    time.sleep(2)
    run('adb shell su -c "service call statusbar 2"')
    time.sleep(2)
    run('adb shell input keyevent "KEYCODE_HOME"')
    time.sleep(2)
    kill_mem_app_process()
    time.sleep(2)
    kill_mem_app_process()
    time.sleep(2)
...
def prepare_for_next_run():
    ...
    print('Closing tab and going to home...')
    run('adb shell input tap 1225 2287')
    time.sleep(10)
    run('adb shell input tap 1354 1366')
    time.sleep(5)
    run('adb shell input keyevent "KEYCODE_HOME"')
    time.sleep(2)

    print('Killing mem app...')
    kill_mem_app()
...
def experiment(run_num, fps, res, mem_state, try_num):
    ...
    # lets clear all apps
    print('Clear all apps...')
    run('adb shell input tap 1121 2488')
    time.sleep(2)
    run('adb shell input swipe 600 1000 600 1700')
    time.sleep(2)
    run('adb shell input swipe 600 1000 600 1700')
    time.sleep(2)
    run('adb shell input swipe 600 1000 600 1700')
    time.sleep(2)
    run('adb shell input tap 1265 187')
    time.sleep(2)
    run('adb shell input keyevent "KEYCODE_HOME"')
    time.sleep(2)
    ...
    run(f'bash set_up_mem_pressure_app.sh {get_mem_to_apply(mem_state)}')
