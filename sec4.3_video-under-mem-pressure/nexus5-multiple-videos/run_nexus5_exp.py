...
def run(cmd):
    ...
    if exit_code != 0:
        if cmd == 'bash nexus5-utils/turn_on_phone.sh':
...
def start_pss_logging(run_num, fps, res, mem_state, q):
    ...
    while q.empty():
        ...
        os.system(f'echo "******************************" >> {file1}')
        os.system(
            f'adb shell dumpsys meminfo org.mozilla.firefox:tab0 >> {file2}')
        os.system(f'date +%s >> {file2}')
        os.system(f'echo "******************************" >> {file2}')
        os.system(
            f'adb shell dumpsys meminfo org.mozilla.firefox:media >> {file3}')
        os.system(f'date +%s >> {file3}')
        os.system(f'echo "******************************" >> {file3}')

        try:
            proc = subprocess.Popen('adb shell ps | grep "org.mozilla.firefox"',
            ...
            pids = [(line.split()[1], line.split()[8])
                    for line in result if line != '' and len(line.split()) >= 9]
            print(pids)
            pid_dict = {}
            for pid in pids:
            ...
            if 'org.mozilla.firefox' in pid_dict:
                os.system(
                    f'adb shell cat /proc/{pid_dict["org.mozilla.firefox"]}/status >> {file11}')
                os.system(f'date +%s >> {file11}')
                os.system(f'echo "******************************" >> {file11}')
            if 'org.mozilla.firefox:tab0' in pid_dict:
                os.system(
                    f'adb shell cat /proc/{pid_dict["org.mozilla.firefox:tab0"]}/status >> {file22}')
                os.system(f'date +%s >> {file22}')
                os.system(f'echo "******************************" >> {file22}')
            if 'org.mozilla.firefox:media' in pid_dict:
                os.system(
                    f'adb shell cat /proc/{pid_dict["org.mozilla.firefox:media"]}/status >> {file33}')
...
def experiment(vid, run_num, fps, res, mem_state, try_num):

    print('======================================================')
    print(
        f'        Run #{run_num} [{fps}fps {res}p | {mem_state}] Try #{try_num}       ')
    print('======================================================')
    ...
    global proc1
    q = Queue()
    args = ['python2', 'robust_mpc_server.py',
            f'{vid}', f'{fps}', f'{res}', f'{run_num}{mem_state[0]}']
    ...
    run('bash nexus5-utils/turn_on_phone.sh')
    ...
    run(
        f'bash nexus5-utils/set_up_mem_pressure_app.sh {get_mem_to_apply(mem_state)}')
    ...
    run(f'bash nexus5-utils/ready_firefox.sh {IP_ADDRESS}')

    # start playing video
    print(f'Starting to play video...')
    run('bash nexus5-utils/press_enter.sh')

    # now video is playing
    # start the timeout thread
    print(f'Starting timeout thread for {TIMEOUT_AFTER_VIDEO_START}s...')
    Thread(target=wait_for_seconds, args=(
        q, TIMEOUT_AFTER_VIDEO_START)).start()

    # we will now wait for either the ABR server to close or the timeout to happen
    print(
        f'Waiting for ABR Server to end or for {TIMEOUT_AFTER_VIDEO_START}s...')
    msg = q.get()
    ...
    if msg == 'secs ended':
        ...
        proc1.send_signal(signal.SIGINT)
        proc1 = None
        experiment(vid, run_num, fps, res, mem_state, try_num + 1)
    # else our video ran smooth (most prolly)!
        ...
        if not has_memory_app_survived():
            ...
            print('======================================================')
            print(
                f'    Success: Run#{run_num} [{fps}fps {res}p | {mem_state}] Try #{try_num}   ')
            print('======================================================')


def set_video(vid_name):
    os.system(f'cp manifest-{vid_name}/* manifest/')


def save_results(vid_name):
    os.system(f'mv results/* runs-{vid_name}/')
                ...
                for fps in [60, 30]:
                    for res in [1080, 720, 480, 360, 240]:
                        for mem_state in ['critical', 'moderate', 'normal']:
                            experiment(vid, run_num, fps, res, mem_state, 1)
