...
public class MySystemService extends Service {
    ...
    String IP_ADDRESS = "192.168.0.188";
...
//                    SaveText(fileName + ".csv", dateFormat.format(date) + "," + list.get(0) + "," + list.get(1) + "," + list.get(2) + "," + list.get(3) + "," + list.get(4) + "," + newVmPressure + "\n");
    ...
    public class constPressureTaskExecutor extends AsyncTask<Integer,Void,Void>
    {
        ...
        final int changeOOMInterval = 1000;
        final int logMemoryInterval = 5000; // should be >= changeOOMInterval
...
//                    List<AndroidAppProcess> processes = AndroidProcesses.getRunningAppProcesses();
//                    for (AndroidAppProcess process : processes) {
//                        if (pids[0] == process.stat().getPid()) {
//                            int proc_oom_score = process.oom_score_adj();
//                            if (proc_oom_score > -1000) {
                                Log.e(TAG, "Changing OOM " + pids[0] + " from " + "proc_oom_score" + " to -1000");
                                String[] cmd = {"su", "-c", "echo -1000 > /proc/" + pids[0] + "/oom_score_adj"};
